...
import { Plus, Search, Pencil, Trash2, Phone, Mail, Building, LayoutGrid, List } from 'lucide-react';
...
export default function ContactsPage() {
  ...
  const totalPages = Math.max(1, Math.ceil((filteredContacts?.length ?? 0) / pageSize));
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      {viewMode === 'table' && (
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          <div>
            Exibindo {paginatedContacts.length} de {filteredContacts.length} contatos
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
                ...
                {(contact?.labels ?? []).length > 0 && (
                  <div className="flex flex-wrap gap-1 pt-2">
                    {(contact?.labels ?? []).map((label: any) => (
                      <Badge
                        key={label?.id}
                        style={{ backgroundColor: label?.color ?? '#000', color: '#fff' }}
                      >
                        {label?.name ?? ''}
                      </Badge>
                    ))}
